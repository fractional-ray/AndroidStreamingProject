...
public class GenreFragment extends Fragment implements FragmentPagerInterface, PopupScrollContainer,SongScrollContainer{
    ...
    {
        View v = inflater.inflate(R.layout.genre_list_fragment_layout,container,false);


        genreRecyclerView = (RecyclerView) v.findViewById(R.id.genreRecyclerView);
    ...
    public void onResumeFragment() {
        List<String> l = main.getSongDB().songDAO().getUniqueGenres();

        genreRecyclerView.setLayoutManager(layoutManager);
        genreAdapter = new GenreAdapter((ArrayList) l,this);
...
//        }
    ...
    public void showPopUp(View view, String id) {
        ...
        ArrayList<Song> a = (ArrayList) main.getSongDB().songDAO().getGenreSongs(id);
        for(Song s:a)
        {
            Log.i("a",s.getSongName());
        }

        r.setAdapter(new SongAdapter(a,this,SongAdapter.GENRE));
    ...
    public void playAndUpdateContext(String id, int toPlay) {
        ...
        main.getPlaybackManager().playAndSwitchContext(new Context(Context.Contexts.GENRE,id,toPlay));
    }
}
